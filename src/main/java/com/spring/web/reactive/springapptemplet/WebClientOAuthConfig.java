...
@Configurationpublic
class WebClientOAuthConfig {
    @Bean
    public WebClient.Builder webClientBuilder(@Qualifier("authorizedClientManager") ReactiveOAuth2AuthorizedClientManager authorizedClientManager) {
        return WebClient.builder().clientConnector(new ReactorClientHttpConnector()).filter(oauth2ExchangeFilterFunction(authorizedClientManager));
    }

    private ExchangeFilterFunction oauth2ExchangeFilterFunction(ReactiveOAuth2AuthorizedClientManager authorizedClientManager) {
        ServerOAuth2AuthorizedClientExchangeFilterFunction oauth2ClientFilter = new ServerOAuth2AuthorizedClientExchangeFilterFunction(authorizedClientManager);
        return ExchangeFilterFunction.ofRequestProcessor(clientRequest -> {                    // This sets the OAuth2 Authorization header in the request                    return Mono.just(clientRequest)                            .flatMap(oauth2ClientFilter::authorize);                });    }
            @Bean
